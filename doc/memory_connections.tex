...
data flows through the arbitration, bank-mapping, and response-routing
logic.
\end{abstract}
...
\texttt{u\_seed\_ram}             & \texttt{seed\_ram}      & 64\,b & 16   & Seed \& Protocol Store \\
\bottomrule
\end{tabular}
\end{table}

\begin{itemize}
  \item Each \textbf{polynomial bank} is a dual-port RAM: Port~A is used
    for reads, Port~B for writes. This allows a simultaneous read and
    write to the same bank in one cycle.
  \item The \textbf{seed RAM} is a single-port synchronous RAM with a
    shared read/write address.
\end{itemize}
...
\node[client, fill=yellow!15, right=1.5cm of pau] (hsu) {HSU\\(Poly Writer)};
\node[client, fill=green!12, right=1.5cm of hsu] (tr) {Transcoder};
\node[client, fill=blue!10, right=1.5cm of tr] (ext) {External\\(Seed User)};

% Arbiter
\node[logic, fill=red!8, minimum width=5cm, below=1.0cm of hsu] (arb)
  {mem\_arbiter\\PAU\,$>$\,HSU\,$>$\,Transcoder};

% Request mux
\node[logic, minimum width=5cm, below=0.6cm of arb] (mux)
  {Request Mux (poly\_mem\_subsystem)};

% Wrapper
\node[logic, fill=cyan!6, minimum width=5cm, below=0.6cm of mux]
  (wrap) {poly\_mem\_wrapper\_4bank\\CMI bank mapping};
...
% Seed RAM
\node[seed, below=0.8cm of wrap, xshift=4.5cm] (sram) {seed\_ram\\64b $\times$ 16};
...
All three clients share the same 4 polynomial banks through the
arbiter. The seed RAM has a separate, un-arbitrated port.}
\label{fig:phy}
\end{figure}
...
through a single arbitrated path. The arbiter grants exactly one client
per cycle; the winner's request is routed through the \emph{same}
\texttt{poly\_mem\_wrapper\_4bank} instance, which applies the same CMI
bank-mapping function regardless of which client is requesting.

The seed RAM is on a \textbf{completely separate port} that bypasses
the polynomial arbiter entirely.
...
\begin{enumerate}
  ...
  \item \textbf{Arbiter resolves priority.} \texttt{mem\_arbiter}
    evaluates all three \texttt{*\_req} signals combinationally and
    produces one-hot grants. Lower-priority requestors are immediately
    stalled.

  \item \textbf{Request mux selects winner.} Inside
    \texttt{poly\_mem\_subsystem}, a combinational mux forwards the
    winning client's signals to the internal memory bus
    (\texttt{poly\_*\_arb}).
  ...
  \item \textbf{Wrapper applies CMI mapping.} For each of the 4~lanes,
    \texttt{poly\_mem\_wrapper\_4bank} computes:
    \begin{align*}
      \text{bank}[i] &= \texttt{cmi\_bank\_idx}(\text{idx}[i]) \\
      \text{addr}[i] &= \texttt{cmi\_bank\_addr}(\text{poly\_id},\;\text{idx}[i])
    \end{align*}

  \item \textbf{Conflict check.} The wrapper performs 6~pairwise
    bank-tag comparisons among active read lanes and 6~among active
    write lanes. If any collision is found, \texttt{ready\_o}~$= 0$
    and the request is rejected (the arbiter propagates a stall to the
    winning client).

  \item \textbf{Bank access.} If no conflict:
    \begin{itemize}
      \item \textbf{Reads} $\to$ Port~A address of the target bank is
        set. Data appears on \texttt{a\_rdata} one cycle later.
      \item \textbf{Writes} $\to$ Port~B write-enable, address, and
        data are applied on the same clock edge.
    \end{itemize}

  \item \textbf{Response routing.} One cycle after a read is accepted,
    \texttt{poly\_mem\_subsystem} uses the saved \texttt{rd\_owner\_q}
    tag to route the wrapper's read response exclusively to the client
    that initiated the read. The other two clients see
    \texttt{*\_rd\_valid}~$= 0$.
\end{enumerate}
...
CWM (Coefficient-Wise Multiply) & 4~coeffs & 4~coeffs &
  Reads 4~coefficients from two polynomials, multiplies
  element-wise, writes 4~accumulated results. \\
...
The PAU accesses all 32 slots depending on the active ML-KEM operation:

\begin{itemize}
  \item \textbf{NTT/INTT:} Operates in-place on any polynomial slot
    (A~matrix rows, s, e, t, or temp).
  \item \textbf{CWM:} Reads from two source slots (e.g., A row and s
    column), writes accumulated result to an output slot (t or temp).
  \item \textbf{ADD drain:} Writes accumulated results from the AU to
    any destination slot.
\end{itemize}
...
The HSU writes coefficients in sequential groups of~4. Under the CMI
...
The HSU is \textbf{primarily a writer}. It may also read (the
\texttt{hsu\_rd\_en} / \texttt{hsu\_rd\_idx} signals exist), but in
typical ML-KEM operation, the HSU's sampling phase is write-only.

\subsubsection{Which Polynomial Slots}

\begin{itemize}
  \item \textbf{Sample NTT:} Writes to A~matrix slots (poly\_id 0--15).
    Each A~matrix entry is sampled using SHAKE-128 rejection sampling.
  \item \textbf{Sample CBD:} Writes to secret~s slots (16--19) and
    error~e slots (20--23) using SHAKE-256 CBD sampling.
\end{itemize}
...
\texttt{poly\_mem\_subsystem} is independent of the polynomial arbiter,
...
then 4, 5, 6, 7, \ldots). Under CMI mapping, each group of~4
...
\begin{itemize}
  \item \textbf{ByteEncode / Compress:} Reads from output~t slots
    (24--27) or A~matrix slots, produces byte stream output.
  \item \textbf{ByteDecode / Decompress:} Writes received ciphertext
    or public-key bytes into temp/scratch slots (28--31) or directly
    into A/t regions.
\end{itemize}
...
The seed RAM has its \textbf{own independent port} on
\texttt{poly\_mem\_subsystem}:

\begin{itemize}
  \item \texttt{seed\_req} / \texttt{seed\_we} / \texttt{seed\_addr} /
    \texttt{seed\_wdata} (inputs)
  \item \texttt{seed\_ready} / \texttt{seed\_rvalid} /
    \texttt{seed\_rdata} (outputs)
\end{itemize}

This port is \textbf{completely independent} of the PAU/HSU/Transcoder
arbiter. It can be used \emph{concurrently} with a polynomial bank
access---an HSU hash operation can read the seed RAM at the same time
the PAU is performing an NTT butterfly on the polynomial banks.

\subsection{Who Drives the Seed Port?}

The memory subsystem does not enforce a specific owner. The seed port
is a passive interface; the system-level integration decides which
module connects to it. In the QREM architecture:
...
Depth        & 2048 entries each             & 16 entries \\
Ports        & Dual-port (A=read, B=write)   & Single-port (shared R/W) \\
Bank mapping & CMI bit-pair-sum              & Direct address \\
Arbitrated   & Yes (PAU$>$HSU$>$Transcoder)  & No (independent port) \\
Data stored  & Polynomial coefficients       & Seeds, hash state,
               protocol metadata \\
Wipe phase   & \texttt{WIPE\_POLY} (2048 cy) & \texttt{WIPE\_SEED} (16 cy) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
  ...
  \item It iterates through all 16~addresses, writing 64'h0 to each.
...
\texttt{WIPE\_POLY} & Banks 0--3 &
  4-lane write (all zeroes) via Port~B &
  $32 \times 64 = 2048$ \\
...
\midrule
\multicolumn{3}{@{}l}{\textbf{Total}} & \textbf{2065 cycles} \\
\bottomrule
\end{tabular}
\end{table}

During \texttt{WIPE\_POLY}, the FSM writes indices
$\{4r, 4r{+}1, 4r{+}2, 4r{+}3\}$ for each row~$r$. Under CMI mapping,
these four indices always map to four distinct banks, so all four
Port~B write-enables fire simultaneously with no conflict.
...
\midrule
\textbf{PAU} &
\multicolumn{4}{c}{R/W via arbiter + CMI adapter (highest priority)} &
--- \\
\textbf{HSU} &
\multicolumn{4}{c}{R/W via arbiter + Poly Stream Writer (mid priority)} &
R (independent port) \\
\textbf{Transcoder} &
\multicolumn{4}{c}{R/W via arbiter (lowest priority)} &
--- \\
\textbf{Controller} &
--- & --- & --- & --- &
W (initial seed load) \\
...
\begin{itemize}
  \item The four polynomial banks are a \textbf{shared resource}. No
    ...
    the same CMI bank-mapping function, and the bank each coefficient
  ...
  \item The seed RAM is \textbf{not} part of the polynomial bank array.
    It is a separate, independent memory on its own port, with no
    arbiter involvement.
  \item Only \textbf{one polynomial-bank transaction} occurs per cycle
    (enforced by the arbiter). However, a seed RAM access can happen
    \textbf{concurrently} with a polynomial bank access.
