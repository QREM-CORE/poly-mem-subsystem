...
\begin{itemize}
  \item HSU-side: \texttt{hsu\_seed\_req}, \texttt{hsu\_seed\_we},
    \texttt{hsu\_seed\_id}, \texttt{hsu\_seed\_idx},
    \texttt{hsu\_seed\_wdata},
  ...
  \item Transcoder-side: \texttt{tr\_seed\_req}, \texttt{tr\_seed\_we},
    \texttt{tr\_seed\_id}, \texttt{tr\_seed\_idx},
    \texttt{tr\_seed\_wdata},
...
During reset or wipe, both seed/protocol ports report not-ready. Seed read data
is valid-qualified: the corresponding \texttt{*\_seed\_rvalid} signal asserts
one cycle after an accepted read, and \texttt{*\_seed\_rdata} is driven to zero
when no valid read response is present. The underlying RAM primitive does not
reset its raw read-data registers; the wipe FSM is responsible for clearing
...
\subsection{Memory-Facing Seed ID + Beat Contract}

At the Memory boundary, the client-facing abstraction is:
...
The physical Seed Store remains address-based. The Memory boundary computes:

\[
  \texttt{seed\_addr = seed\_base\_addr(seed\_id) + seed\_idx}
\]

This keeps the RAM primitive simple while removing magic-address burden from
HSU and Transcoder designers.
