...
read+write request, it owns both internal ports atomically. Every issued
...
The HSU is a \textbf{polynomial-memory writer}. The \texttt{hsu\_rd\_*}
signals remain at the top-level interface for stability, but in the current
Memory implementation HSU polynomial reads are intentionally unsupported.
HSU reads protocol objects such as seeds through the dedicated
\texttt{seed\_*} port instead.
