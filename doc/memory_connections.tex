...
arbiter. The seed RAM sits on a separate dual-port path and is not
arbitrated with polynomial traffic.}
\label{fig:phy}
\end{figure}
...
through a single arbitrated path. The subsystem grants one
\textbf{read owner} and one \textbf{write owner} per cycle; the winner's
...
bypasses the polynomial arbiter entirely.
...
polynomial arbiter. They can be used \emph{concurrently} with polynomial-bank
traffic.
...
Ports        & Dual-port (A=read, B=write)   & Dual-port (HSU + Transcoder) \\
...
\begin{itemize}
  ...
  \item The seed RAM is \textbf{not} part of the polynomial bank array.
    ...
    arbiter involvement.
  \item The polynomial subsystem supports one \textbf{read owner} and one
    \textbf{write owner} per cycle. A read-only client can therefore overlap
    with a write-only client, while a combined read+write request owns both
    planes atomically.
