...
mapping. A deterministic internal two-port scheduler maps up to two legal
...
\node[block, fill=cyan!10, minimum width=5.5cm, minimum height=1.5cm,
      below=0.5cm of mux]
  (wrap) {poly\_mem\_wrapper\_4bank\\2 Generic Ports\\CMI + Hazard Checks};
...
% ────────────────────────────────────────────────────────────
\subsection{\texttt{mem\_arbiter} --- Legacy Priority Helper}
...
The current \texttt{poly\_mem\_subsystem} no longer instantiates this
module directly. The active top-level scheduler is implemented inline so it
can reason about pair legality before admitting a second request.
