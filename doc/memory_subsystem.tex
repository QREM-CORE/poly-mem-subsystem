...
and Transcoder---onto the shared memory plane, while a dedicated dual-port
seed/protocol store and a security-wipe finite state machine complete the design. The
...
Read-vs-write to the same bank is \textbf{not} automatically a conflict because
reads use Port~A and writes use Port~B of the dual-port RAM. However,
\textbf{same-address read+write is explicitly forbidden} in the current
v0.75-alignment pass, and same-address write+write is also forbidden. These
conditions raise \texttt{mem\_fault\_o} with an encoded
\texttt{mem\_fault\_code\_o}.
...
\begin{itemize}
  \item A \textbf{four-bank dual-port polynomial memory} with CMI
    ...
    access.
  \item A \textbf{strict-priority split-plane arbiter} (PAU~$>$~HSU~$>$~Transcoder)
    granting one read owner and one write owner per cycle, with atomic
