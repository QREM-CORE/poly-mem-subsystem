...
\begin{itemize}
  ...
  \item At the \texttt{poly\_mem\_subsystem} boundary, seed ports report
    not-ready during reset or wipe, and read data is meaningful only when the
    matching \texttt{*\_seed\_rvalid} signal is asserted.
  \item The raw RAM primitive does not reset memory contents or read-data
    registers; this preserves BRAM-friendly inference, while the top-level
    boundary drives idle read data to zero.
\end{itemize}

At the \texttt{poly\_mem\_subsystem} boundary, seed/protocol accesses are
described as \texttt{seed\_id + seed\_idx} (beat offset). The physical Seed
Store remains address-based; Memory computes the raw RAM address internally:
